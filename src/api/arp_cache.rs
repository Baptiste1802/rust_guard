...
impl ArpCache {
    ...
    pub fn insert(&mut self, ip: Ipv4Addr, mac: String) {
        match self.cache.contains_key(&ip) {
            true => {
                println!("Pair IP/MAC valide")
            }
            false => {
                println!("Pair IP/MAC invalide")
            }
        };
    ...
    fn network_verification(&mut self, packet: &PacketInfos) -> Result<(), String> {
        ...
        match packet.get_layer_3_infos() {
            Layer3Infos::ARP(arp_handler) => {
                ...
                } else if (arp_handler.operation == ArpOperations::Request)
                    && (arp_handler.hw_source.is_broadcast())
                {
...
mod tests {
    ...
    fn paquet_generator<'a>(
        ether_mac_source: MacAddr,
        arp_mac_source: MacAddr,
        mac_dest: MacAddr,
        ip_source: Ipv4Addr,
        ip_dest: Ipv4Addr,
    ...
    fn test_network_verification() {
...

        {
            ...
            let fake_paquet_info: PacketInfos =
                PacketInfos::new(&interface_name, &ethernet_packet.to_immutable());
...

        {
            ...
            let fake_paquet_info: PacketInfos =
                PacketInfos::new(&interface_name, &ethernet_packet.to_immutable());
            assert_eq!(
                cache.network_verification(&fake_paquet_info),
                Err("handle alert -> IP source not valid".to_string())
            );
        }
...

        {
            ...
            assert_eq!(
                cache.network_verification(&fake_paquet_info),
                Err("handle alert -> Ip source not in subnet".to_string())
            );
        }
    }
}
